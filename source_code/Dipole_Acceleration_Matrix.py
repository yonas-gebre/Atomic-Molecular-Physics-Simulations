...
def Dipole_Acceleration_Z_Matrix(input_par, grid, wigner_3j_dict):
    ...
    for i in range(istart, iend):
        ...
        if l_block < input_par["l_max"]:
            columon_idx = grid.size*qn_to_block[(l_block + 1, m_block)] + grid_idx
            matrix_element  = (l_block+1)/sqrt((2*l_block+1)*(2*l_block+3))/grid2[grid_idx]
            Dipole_Acceleration_Matrix.setValue(i, columon_idx, matrix_element)
            
        if abs(m_block) < l_block and l_block > 0:
            columon_idx = grid.size*qn_to_block[(l_block - 1, m_block)] + grid_idx
            matrix_element = l_block/sqrt((2*l_block-1)*(2*l_block+1))/grid2[grid_idx]
            Dipole_Acceleration_Matrix.setValue(i, columon_idx, matrix_element)
...
def Dipole_Acceleration_X_Matrix(input_par, grid, wigner_3j_dict):
    ...
    for i in range(istart, iend):
        ...
        if l_block < input_par["l_max"]:
            ...
            common_factor = pow(-1.0, m_block)*sqrt((2*l_block+1)*(2*l_prime+1)/2)*wigner_3j_dict[(l_block,1,l_prime,0,0,0)]
            

            m_prime = m_block - 1
            wigner_3j_dict = Check_Dictionary_Key_For_W3J_Symbol(wigner_3j_dict, (l_block,1,l_prime,-1*m_block,1,m_prime))
            wigner_3j_dict = Check_Dictionary_Key_For_W3J_Symbol(wigner_3j_dict, (l_block,1,l_prime,-1*m_block,-1,m_prime))
            matrix_element = common_factor * (wigner_3j_dict[(l_block,1,l_prime,-1*m_block,-1,m_prime)] - wigner_3j_dict[(l_block,1,l_prime,-1*m_block,1,m_prime)])/grid2[grid_idx]
            columon_idx = grid.size*qn_to_block[(l_prime, m_prime)] + grid_idx
            Dipole_Acceleration_Matrix.setValue(i, columon_idx, matrix_element)
...
def Dipole_Acceleration_Y_Matrix(input_par, grid, wigner_3j_dict):
    ...
    for i in range(istart, iend):
        ...
        if l_block < input_par["l_max"]:
            l_prime = l_block + 1
            wigner_3j_dict = Check_Dictionary_Key_For_W3J_Symbol(wigner_3j_dict, (l_block,1,l_prime,0,0,0))
            common_factor = 1.0j*pow(-1.0, m_block)*sqrt((2*l_block+1)*(2*l_prime+1)/2)*wigner_3j_dict[(l_block,1,l_prime,0,0,0)]
